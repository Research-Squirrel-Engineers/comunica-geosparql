...
export class TermFunctionAsPLY extends TermFunctionBase {
  public constructor() {
    super({
      ...
      overloads: declare(GeoSparqlExtOperator.ASPLY).onGeometry1(() => term => {

        //const result=await new GeoJSON()
        //serializeGeometry(term, 'http://www.opengis.net/ont/geosparql#plyLiteral')
        return string("");
      }).collect(),
    });
  }
